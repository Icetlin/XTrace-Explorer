...
def capture_all(page, base_url: str, file_id: int, ss_dir: Path) -> dict[str, str]:
    ...
    # 3. Open the demo trace file — find .file-row containing "demo"
    try:
        demo_row = page.locator(".file-row").filter(has_text="demo")
        if demo_row.count() == 0:
            # fallback: first row
    ...
    # 5. Expand first listener → call tree (CallNode children)
    try:
        # wait for listener-row to be visible (event was expanded in step 4)
        page.wait_for_selector(".listener-row", timeout=5000)
        page.locator(".listener-row").first.click()
        time.sleep(3.0)
        page.wait_for_load_state("networkidle")
        page.wait_for_selector(".children .call-node", timeout=15000)
    ...
    # 6. Expand a non-leaf call-node deeper
    try:
        # call-node with children has a chevron; find one that can expand
        expandable = page.locator(".call-node .call-row").filter(has_not_text="").first
        expandable.click()
        time.sleep(0.8)
        page.wait_for_load_state("networkidle")
    ...
    # 7. Code view — click a call-row with a file path to open the split code panel
    try:
        call_rows = page.locator(".call-row").all()
        for row in call_rows[:10]:
            row.click()
            time.sleep(0.6)
            page.wait_for_load_state("networkidle")
            if page.locator(".code-view").count() > 0:
                shots["07_code_view"] = wait_and_screenshot(page, str(ss_dir / "07-code-view.png"))
                break
        if "07_code_view" not in shots:
            print("[readme-updater] skip 07_code_view: no code-view appeared", flush=True)
    except Exception as e:
        print(f"[readme-updater] skip 07_code_view: {e}", flush=True)
    ...
    # disable CSS transitions on float-ctrl so buttons become stable for clicking
    page.evaluate("""() => {
        const el = document.querySelector('.float-ctrl');
        if (el) el.style.transition = 'none';
        document.querySelectorAll('.float-ctrl *').forEach(e => e.style.transition = 'none');
    }""")
    time.sleep(0.2)

    # 9. Light theme — click the theme toggle button (.float-ctrl__item--theme)
    try:
        page.locator(".float-ctrl__item--theme").click(force=True)
        time.sleep(0.5)
        shots["08_light_theme"] = wait_and_screenshot(page, str(ss_dir / "08-light-theme.png"))
        # switch back to dark
        page.locator(".float-ctrl__item--theme").click(force=True)
        time.sleep(0.3)
    except Exception as e:
        print(f"[readme-updater] skip 08_light_theme: {e}", flush=True)

    # 10. Xdebug mode panel — click the bug icon to show mode options
    try:
        page.locator(".float-ctrl__item--xd").click(force=True)
        time.sleep(0.4)
        page.wait_for_selector(".xd-options", timeout=4000)
        shots["10_xdebug_panel"] = wait_and_screenshot(page, str(ss_dir / "10-xdebug-panel.png"))
        # close it
        page.locator(".float-ctrl__item--xd").click(force=True)
        time.sleep(0.3)
    except Exception as e:
        print(f"[readme-updater] skip 10_xdebug_panel: {e}", flush=True)

    # 11. Settings modal — click the gear icon in float-ctrl
    try:
        page.locator(".float-ctrl__item[title='Settings']").click(force=True)
        time.sleep(0.5)
        page.wait_for_selector(".settings-page", timeout=6000)
        shots["09_settings"] = wait_and_screenshot(page, str(ss_dir / "09-settings.png"))
        # Xdebug tab inside settings
        xd_tab = page.locator(".nav-item").filter(has_text="Xdebug")
        if xd_tab.count() > 0:
            xd_tab.first.click(force=True)
            time.sleep(0.4)
            shots["09b_xdebug"] = wait_and_screenshot(page, str(ss_dir / "09b-xdebug.png"))
        # close modal
        page.locator(".float-modal__close").click(force=True)
        time.sleep(0.3)
    except Exception as e:
...
def build_gifs(shots: dict[str, str], ss_dir: Path) -> dict[str, str]:
    ...
    deep_keys = ["05_calltree", "05b_calltree_deep", "07_code_view"]
...
def main():
    ...
    # Auto-detect a ready file_id — prefer demo_shop
    file_id = args.file_id
    if not file_id:
        import httpx
        files = httpx.get(f"{args.app_url}/api/files", timeout=10).json()
        ready = [f for f in files if f.get("status") == "ready"]
        # prefer demo_shop trace for screenshots (compact, has Acme\ namespace)
        demo = [f for f in ready if "demo_shop" in f.get("name", "").lower()]
        chosen = demo[0] if demo else (ready[0] if ready else None)
        if chosen:
            file_id = chosen["file_id"]
            print(f"[readme-updater] using file_id={file_id} ({chosen['name'][:60]})", flush=True)
